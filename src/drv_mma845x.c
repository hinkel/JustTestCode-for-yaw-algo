...
bool mma8452Detect(sensor_t *acc)
{
    ...
    if (!ack || (sig != MMA8452_DEVICE_SIGNATURE && sig != MMA8451_DEVICE_SIGNATURE)) return false;
...
static void mma8452Read(int16_t *accelData)
{
    ...
    accelData[0] = (int16_t)(((int16_t)buf[0] << 8) | buf[1]) >> 4;
    accelData[1] = (int16_t)(((int16_t)buf[2] << 8) | buf[3]) >> 4;
    accelData[2] = (int16_t)(((int16_t)buf[4] << 8) | buf[5]) >> 4;
}
