...
void readEEPROM(void)
{
    ...
    cfg.tri_ymid = constrain(cfg.tri_ymid, cfg.tri_ymin, cfg.tri_ymax);                   // REAR
...
// TESTCODE START
    ...
    cfg.magZero[0]                = -103.0f;
    cfg.magZero[1]                =  -50.0f;
    cfg.magZero[2]                = -234.0f;
...
//  cfg.mag_dec                   = 0;          // For example, -6deg 37min, = -637 Japan, format is [sign]dddmm (degreesminutes) default is zero.
    cfg.mag_dec                   = 111;        // Crashpilot //cfg.acc_hdw = ACC_DEFAULT;// default/autodetect
...
//    cfg.acc_lpfhz                 = 0.536f;     // Acc LPF in Hz for angle/horizon
//    cfg.acc_lpfhz                 = 0.70f;      // Acc LPF in Hz for angle/horizon
    cfg.acc_lpfhz                 = 2.0f;       // Acc LPF in Hz for angle/horizon
    cfg.acc_altlpfhz              = 10.0f;      // LPF in HZ just for althold
    cfg.acc_gpslpfhz              = 6.0f;       // LPF in HZ just for GPS ins stuff
    cfg.looptime                  = 3000;
    cfg.mainpidctrl               = 0;          // 0 = OriginalMwiiPid pimped by me, 1 = New mwii controller (experimental, float pimped + pt1)
    cfg.maincuthz                 = 12;         // (1-100Hz) Cuf Off Frequency for pt2 element D term in Hz of main Pid controller
...
//    cfg.gy_cmpf                   = 1000;       // (10-1000) 400 default. Now 1000. The higher, the more weight gets the gyro and the lower is the correction with Acc data.
    cfg.gy_cmpf                   = 800;        // (10-1000) 400 default. Now 1000. The higher, the more weight gets the gyro and the lower is the correction with Acc data.
    cfg.gy_cmpfm                  = 1500;       // (10-2000) 200 default for 10Hz. Now higher. Gyro/Magnetometer Complement.
    cfg.gy_smrll                  = 0;
    cfg.gy_smptc                  = 0;
    ...
    cfg.accz_acf                  = 0.940f;     // Crashpilot: Value for complementary filter accz and altitude
//  cfg.accz_vcf                  = 0.988f;     // Crashpilot: Value for complementary filter accz and barovelocity
//  cfg.accz_acf                  = 0.988f;     // Crashpilot: Value for complementary filter accz and altitude
