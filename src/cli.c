...
extern float   GyroNoise;                   // Populated upon initialization
...
const clivalue_t valueTable[] =
{
    ...
    { "acc_lpfhz",                 VAR_FLOAT,  &cfg.acc_lpfhz,                   0,         50, 1 },
    { "acc_altlpfhz",              VAR_FLOAT,  &cfg.acc_altlpfhz,                0,         50, 1 },
    { "acc_gpslpfhz",              VAR_FLOAT,  &cfg.acc_gpslpfhz,                0,         50, 1 },
    { "gy_lpf",                    VAR_UINT16, &cfg.gy_lpf,                      0,        256, 0 },
    { "gy_cmpf",                   VAR_UINT16, &cfg.gy_cmpf,                    10,       8000, 1 },
    { "gy_cmpfm",                  VAR_UINT16, &cfg.gy_cmpfm,                   10,       8000, 1 },
...
static void cliStatus(char *cmdline)
{
    ...
    printf("\r\nNoise*10: %d",(int32_t)(GyroNoise * 10.0f));
    if (cfg.ShakyDataAvail)
    {
        printf("\r\nFallback");
        printxyzcalval(cfg.ShakyGyroZero);
        printf("\r\nNoise*10: %d",(int32_t)(cfg.ShakyGyroNoise * 10.0f));
    }
    ...
    if (sensors(SENSOR_ACC))
    {
        ...
        if(cfg.acc_calibrated)
        {
            ...
            printf("\r\n1G val: %d", (int32_t)cfg.sens_1G);
            printf("\r\nNoise: %d", (int32_t)(cfg.accNoise));
        }
        else cliRecal();
    }
