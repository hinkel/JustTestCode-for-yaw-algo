...
bool GPS_alltime(void)
{
    ...
    if (GPS_FIX && GPS_numSat >= 5)                                             // Do gps stuff with at least 5 Sats
    {
        ...
        dTnav = ACCDeltaTimeINS;                                                // dTnav in "seconds"
        dTnav = min(dTnav, 1.0f);                                               // This prevents crazy dt on first run
...
static void GPS_HzSandbox(void)
{
    static uint32_t PosHoldBlindTimer = 0, PhTimer1 = 0, AvgSpeed = 0;
    static bool     PHtoofast, PHChange;
    static uint8_t  PHcascade;
    uint16_t        speed;
    ...
    if (DoingGPS())
    {
        ...
        case NAV_MODE_POSHOLD:
            ...
            case 2:
                ...
                break;
            case 3:
                if(GPS_update)                                                 // Is true every 2nd new GPS data!
                {
                    AvgSpeed += (GPS_speed - AvgSpeed) / 2;                    // Keep averaging speed
                    if (AvgSpeed < cfg.gps_ph_settlespeed) PHcascade = 4;
                    else if(!PhTimer1) PhTimer1 = currentTimeMS + constrain(((AvgSpeed - cfg.gps_ph_settlespeed) / cfg.gps_ph_brkacc) * 1000, 1, 5000);  // t = v/a limit to 5 seconds
                    if (PhTimer1 && currentTimeMS >= PhTimer1) PHcascade = 4;
                }
                break;
