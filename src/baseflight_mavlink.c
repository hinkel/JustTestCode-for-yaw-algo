...
void baseflight_mavlink_handleMessage(mavlink_message_t *msg)
{
    ...
    {
/*      
    case MAVLINK_MSG_ID_COMMAND_LONG:                          // Check for some preflightstuff DONT KNOW WHERE IT IS SUPPORTED
    {
        mavlink_command_long_t packet;
        mavlink_msg_command_long_decode(msg, &packet);
        if (f.ARMED) break;                                    // This is all PREFLIGHT stuff coming, just to be save check it here and abort when already armed
        switch(packet.command)
        {
        case MAV_CMD_PREFLIGHT_STORAGE:                        // Just check savecommand here, not the read command
            if (packet.param1 == 1.0) cliSave("");
            break;
        case MAV_CMD_PREFLIGHT_CALIBRATION:                    // if (packet.param1 == 1.0) calibratingG = true;
            if (packet.param2 == 1.0) calibratingA = true;
            if (packet.param3 == 1.0) calibratingM = true;
            break;
        }
        break;
    }
