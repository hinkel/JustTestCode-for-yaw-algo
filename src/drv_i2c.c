...
//        SR2Register = I2Cx->SR2;                             // read second status register to clear ADDR if it is set (note that BTF will not be set after a NACK)
        I2Cx->SR2;                                           // read second status register to clear ADDR if it is set (note that BTF will not be set after a NACK)
...
void i2c_ev_handler(void)
{
    ...
    if (SReg_1 & 0x0001)                                     // we just sent a start - EV5 in ref manual
    {
        ...
        else                                                 // direction is Tx, or we havent sent the sub and rep start
        {
            ...
            if (reg != 0xFF) index = -1;                     // 0xFF as subaddress means it will be ignored, in Tx or Rx mode // send a subaddress
        }
    }
...
//            a = I2Cx->SR2;                                   // clear the ADDR here
    ...
    else if (SReg_1 & 0x004)                                 // Byte transfer finished - EV7_2, EV7_3 or EV8_2
    {
        ...
        if (reading && subaddress_sent)                      // EV7_2, EV7_3
        {
            ...
            {
                if (final_stop) I2C_GenerateSTOP(I2Cx, ENABLE);// program the Stop
                else I2C_GenerateSTART(I2Cx, ENABLE);         // program a rep start
                read_p[index++] = I2C_ReceiveData(I2Cx);     // read data N-1
    ...
    else if (SReg_1 & 0x0040)                                // Byte received - EV7
    {
        ...
        if (bytes == (index + 3)) I2C_ITConfig(I2Cx, I2C_IT_BUF, DISABLE); // disable TXE to allow the buffer to flush so we can get an EV7_2
        if (bytes == index) index++;                         // We have completed a final EV7 // to show job is complete
    }
    else if (SReg_1 & 0x0080)                                // Byte transmitted -EV8/EV8_1
    {
        if (index != -1)                                     // we dont have a subaddress to send
        {
            ...
            if (bytes == index) I2C_ITConfig(I2Cx, I2C_IT_BUF, DISABLE); // we have sent all the data // disable TXE to allow the buffer to flush
        }
        else
        {
            index++;
            I2C_SendData(I2Cx, reg);                         // send the subaddress
            if (reading || !bytes) I2C_ITConfig(I2Cx, I2C_IT_BUF, DISABLE); // if receiving or sending 0 bytes, flush now // disable TXE to allow the buffer to flush
        }
    }
...
static void i2cUnstick(void)
{
    ...
    for (i = 0; i < 8; i++)
    {
        ...
        while (!digitalIn(GPIOB, Pin_10)) delayMicroseconds(10);
