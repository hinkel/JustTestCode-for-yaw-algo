...
- Scaled Level I for normal PID controller down by 10! So former "10" would be "100" now.
...
- Removed moving average filter from D calculation. Therefor maincuthz (default 10Hz, Range 5-100Hz) becomes critical for D calculation. Higher frequency lets more ripple pass in D part.
...
-- acc_lpf  = 100 deleted and repaced by acc_lpfhz  = 0.7 (0.536 is equivalent to the former "100" based on 3ms cycletime)
-- acc_ilpf =  10 deleted and repaced by acc_altlpfhz = 10.0 (5.895 is equivalent to the former " 10" based on 3ms cycletime)
-- acc_gpslpfhz is separate lpf for GPS ins. Default is 5.5 Hz for now. (GPS not tested)
-- Measure gyro/acc sensor noise upon calibration. Recalibrate acc upon change of gy_lpf or acc type of course.
...
-- The gyrocalibration data is saved upon acc calibration because if you desperately try to arm your copter on shaky conditions(boats, earthquakes..),
   after 21 seconds of frustrane gyro calibration those already saved data will be taken just to be flyable.
...
- Reworked ACC & GYRO Calibration to Sphere Algo. "gy_stdev" is set to more sensitive. Acc Calibration takes longer now (more values).
...
- Status Spits out a great deal of calibration data, mag, baro, acc/gyro info, shows unused/free bytes in eeprom.
